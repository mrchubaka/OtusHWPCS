...
###Подготовка каждого хоста кластера по сети + настройки GRUB согласно документации Pivotal
```
Установка hostname
...
###Загрузка данных из базы PG, что осталась от лабы 7.
```
alex=# CREATE FOREIGN TABLE taxi_trips_fdw_pg (
    unique_key text,
    taxi_id text,
